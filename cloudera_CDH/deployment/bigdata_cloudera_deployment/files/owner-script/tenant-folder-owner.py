    ...
    for sensor in sensor_section:
        ...
        for stream in sensor["streams"]:
            ...
            for tenant in tenant_section:
                ...
                tenant_folder_user_owner = tenant["folder_permission"]["user_owner"]
                tenant_folder_group_owner = tenant["folder_permission"]["group_owner"]



                command = tenant_folder_user_owner + ':' + tenant_folder_group_owner + ' ' + '/' + os.path.join(root_folder_name, repository_folder_name,sensor_prefix + sensor_folder_name + stream_prefix + stream_folder_name,tenant_prefix + tenant_folder_name)
                tenant_owner_commands.append(command)

#print(len(tenant_owner_commands))
print(tenant_owner_commands)
file.close()
