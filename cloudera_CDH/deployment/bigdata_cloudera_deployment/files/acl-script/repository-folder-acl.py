...
for repository in repository_section:
    ...
    for acl_user_info in repository.get("acl_users") or []:
        acl_user = acl_user_info["user"]
        acl_user_permission = acl_user_info["permission"]

        command = 'user' + ':' + acl_user + ':' + acl_user_permission + ' ' + '/' + os.path.join(root_folder_name,repository_folder_name)
        repository_acl_commands.append(command)


    for acl_group_info in repository.get("acl_groups") or []:
        acl_group = acl_group_info["group"]
        acl_group_permission = acl_group_info["permission"]

        command = 'group' + ':' + acl_group + ':' + acl_group_permission + ' ' + '/' + os.path.join(root_folder_name,repository_folder_name)
        repository_acl_commands.append(command)



print(repository_acl_commands)
file.close()
