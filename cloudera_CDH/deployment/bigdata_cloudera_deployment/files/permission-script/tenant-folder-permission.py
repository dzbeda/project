    ...
    for sensor in sensor_section:
        ...
        for stream in sensor["streams"]:
            ...
            for tenant in tenant_section:
                ...
                tenant_folder_permission = tenant["folder_permission"]["permission"]


                command = str(tenant_folder_permission) + ' ' + '/' + os.path.join(root_folder_name, repository_folder_name,sensor_prefix + sensor_folder_name + stream_prefix + stream_folder_name,tenant_prefix + tenant_folder_name)
                tenant_permission_commands.append(command)

#print(len(tenant_permission_commands))
print(tenant_permission_commands)
file.close()
