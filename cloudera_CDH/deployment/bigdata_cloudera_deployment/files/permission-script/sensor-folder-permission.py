    ...
    if repository_folder_name == "raw":
        ...
        for sensor in sensor_section:
            ...
            for stream in sensor["streams"]:
                ...
                stream_folder_permission = stream["folder_permission"]["permission"]


                command = str(stream_folder_permission) + ' ' + '/' + os.path.join(root_folder_name, repository_folder_name,sensor_prefix + sensor_folder_name + stream_prefix + stream_folder_name)
                sensor_permission_commands.append(command)

#print(len(sensor_permission_commands))
print(sensor_permission_commands)
file.close()
